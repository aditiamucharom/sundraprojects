...
export default function TaskList({
  ...
  // Load workspace projects (lists) for selection when creating tasks outside any specific list
  useEffect(() => {
  ...
  const handleQuickAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // fallback to the chosen project, or default to the top-most workspace project.
      if (!finalProjectId) {
        if (newTaskProjectId) {
          finalProjectId = newTaskProjectId;
        } else {
          const workspaceProjects = await dataService.getProjects(activeWorkspace.id);
      ...
      const { data: createdTask, error } = await dataService.createTask(
        activeWorkspace.id,
        finalProjectId!,
        newTaskTitle.trim(),
        '',
        ...
        <form onSubmit={handleQuickAdd} className="space-y-3">
          ...
          {isQuickAddExpanded && (
            <div className="pl-0 sm:pl-8 pt-2 flex flex-wrap items-center gap-2 sm:gap-3 border-t border-border-custom/50 animate-backdrop-fade-in w-full">
              ...
              {/* List Selector Option (shown only when not already inside a specific project/list) */}
              {!projectId && (
                <div className="flex items-center space-x-1.5 bg-background border border-border-custom px-2.5 py-1.5 rounded-lg text-xs w-[calc(50%-4px)] sm:w-auto min-w-0">
                  <ListIcon className="h-3.5 w-3.5 text-text-secondary flex-shrink-0" />
                  <select
                    value={newTaskProjectId}
                    onChange={(e) => setNewTaskProjectId(e.target.value)}
                    className="bg-transparent border-0 focus:ring-0 p-0 text-xs text-foreground focus:outline-none cursor-pointer flex-1 w-full sm:w-36 min-w-0"
                  >
                    <option value="">
                      {projects[0] ? `List: ${projects[0].name}` : 'Default List'}
                    </option>
                    {projects.map((p) => (
